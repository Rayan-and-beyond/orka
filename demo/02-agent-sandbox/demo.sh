...
peq "gh pr list --repo sozercan/orka-demo-inventory --head orka/healthz-from-sandbox --json number --jq '.[].number' | xargs -I{} gh pr close {} --repo sozercan/orka-demo-inventory --delete-branch"
peq "git ls-remote --exit-code $DEMO_REPO refs/heads/orka/healthz-from-sandbox && git push $DEMO_REPO --delete orka/healthz-from-sandbox"
ensure_port_forward
orka_connect
...
banner "Orka — an agent workspace that outlives the agent" \
  "Codex works across two turns in one Agent Sandbox. Between them the Sandbox sleeps; its files do not."
...
chapter "Turn 1 — read the code, write a plan"

pe "sed -n '1,25p' $here/manifests/turn-1-plan.yaml"
pe "orka task create -f $here/manifests/turn-1-plan.yaml"
...
pe "kubectl -n $runtime_ns exec $pod -c agent -- env | grep -ciE 'token|api_key|secret' || echo 'no credentials in the environment'"
say "Now the agent works. Orka records what it does as execution events."
wait_task inventory-plan 1200
pe "orka task events inventory-plan | grep -E 'ToolCall|AgentRuntime' | tail -n 8"
pe "orka task result inventory-plan"
ok "PLAN.md is on the Sandbox's disk. Nothing was pushed: this turn was read-only."

chapter "The Sandbox sleeps"

ws=$(workspace_of inventory-plan)
...
pe "kubectl -n $runtime_ns get sandbox $sb -o jsonpath='{.spec.operatingMode}'; echo"
pe "kubectl -n $runtime_ns get pods,pvc"
ok "No agent Pod is running. The volume with PLAN.md is Bound and waiting."

chapter "Turn 2 — same Session, same disk, real change"

say "A second Task in the same Session. Write intent this time, with the"
say "Publisher's credentials named so Orka can open the pull request."
pe "sed -n '35,54p' $here/manifests/turn-2-implement.yaml"
pe "orka task create -f $here/manifests/turn-2-implement.yaml"
wait_for "the Sandbox to wake" \
  "[[ \$(kubectl -n $runtime_ns get sandbox $sb -o jsonpath='{.spec.operatingMode}') == Running ]]" 600
...
pe "orka task result inventory-implement | head -n 30"
say "The agent read its own plan from the previous turn and implemented it. It"
say "did not push: Orka's Publisher verified the tree, published the branch,"
say "and opened the pull request."
pe "orka task status inventory-implement"

chapter "The pull request"

pr=$(gh pr list --repo sozercan/orka-demo-inventory --head orka/healthz-from-sandbox --json url --jq '.[0].url')
assert_pr "$pr"
pe "gh pr view $pr --json title,url,headRefName --jq '{title,url,branch:.headRefName}'"
pe "gh pr diff $pr --stat"
ok "Two turns, one Sandbox, one pull request. The plan file never left the cluster."
